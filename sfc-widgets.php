...
function get_sfc_activity_feed($args) {
	...
	return "<fb:activity site='{$site}' width='{$width}' height='{$height}' header='{$header}' colorscheme='{$colorscheme}' font='{$font}' border_color='{$bordercolor}' recommendations='{$recommendations}'></fb:activity>";
}
...
<fb:login-button v="2" perms="read_stream" onlogin="sfcUserStatusSetup('<?php echo 'widget-'.$this->id_base.'-'.$this->number; ?>')">Grant Permissions</fb:login-button>
<?php
	}
}
...
function sfcUserStatusSetup(base) {
	FB.getLoginStatus(function(response) {
		if (response.session) {
			jQuery('#'+base+'-profileid').val(response.session.uid);
			jQuery('#'+base+'-access_token').val(response.session.access_token);
		} else {
